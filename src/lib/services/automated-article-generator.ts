...
class AutomatedArticleGenerator {
  ...
  public async processScheduledJobs(): Promise<void> {
    ...
    console.log(`🔍 Looking for any pending job that should be generated by now (current hour: ${currentHour})`);

    // Find any pending job that should have been generated by now
    // Look for jobs scheduled for current hour or earlier
    const pendingJobs = dailyPlan.jobs.filter(job => job.status === 'pending');

    if (pendingJobs.length === 0) {
      console.log(`✅ No pending jobs found - all articles generated for today`);
      return;
    }

    // Find the earliest pending job that should be processed
    ...
    for (const job of pendingJobs) {
      ...
      // Process job if it's scheduled for current hour or earlier
      if (scheduledHour <= currentHour) {
        jobToProcess = job;
        console.log(`🎯 Found overdue job #${job.position} for hour ${scheduledHour}: "${job.trend.title}"`);
        break;
      }
    }

    // If no overdue job, take the next scheduled job
    if (!jobToProcess && pendingJobs.length > 0) {
      jobToProcess = pendingJobs[0];
      const scheduledTime = new Date(jobToProcess.scheduledAt!);
      const scheduledHour = scheduledTime.getHours();
      console.log(`🎯 No overdue jobs, processing next scheduled job #${jobToProcess.position} for hour ${scheduledHour}: "${jobToProcess.trend.title}"`);
    }

    if (!jobToProcess) {
      console.log(`✅ No suitable job found for processing`);
      return;
    }

    const currentHourJob = jobToProcess;

    // Process the job immediately - no time restrictions for testing
    ...
    console.log(`🚀 Processing job immediately (no time restrictions)`);

    console.log(`🎯 Starting article generation: #${currentHourJob.position} "${currentHourJob.trend.title}"`);
